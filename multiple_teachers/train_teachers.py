...
def train_teacher(dataset, nb_teachers, teacher_id):
  ...
  # Retrieve subset of data for this teacher检索每个教师的数据子集
  ...
  # Define teacher checkpoint filename and full path定义教师检查点文件名和完整路径
  if FLAGS.deeper:
  ...
  # Perform teacher training 进行教师培训
  assert deep_cnn.train(data, labels, ckpt_path)

  # Append final step value to checkpoint for evaluation 将最后的步骤值附加到检查点以进行评估
  ckpt_path_final = ckpt_path + '-' + str(FLAGS.max_steps - 1)

  # Retrieve teacher probability estimates on the test data 检索测试数据的教师概率估计
