...
def maybe_download(file_urls, directory):
  """
  Download a set of files in temporary local folder#在临时本地文件夹中下载一组文件
  :param directory: the directory where to download
  :return: a tuple of filepaths corresponding to the files given as input#对应于输入的文件的一组文件路径
  """
  # Create directory if doesn't exist
  assert create_dir_if_needed(directory)#用以检查某一条件是否为True，若该条件为False则会给出一个AssertionError

  # This list will include all URLS of the local copy of downloaded files此列表将包括下载文件的本地副本的所有url
  result = []
  ...
  for file_url in file_urls:
    ...
    # Deduce local file url 推导出本地文件的url
    ...
    # Test if file already exists测试文件是否已经存在
    if not tf.gfile.Exists(filepath):
      def _progress(count, block_size, total_size):
        sys.stdout.write('\r>> Downloading %s %.1f%%' % (filename,
            float(count * block_size) / float(total_size) * 100.0))
        sys.stdout.flush()
      filepath, _ = urllib.request.urlretrieve(file_url, filepath, _progress)#复制由URL指向本地文件的网络对象。如果URL指向本地文件，除非提供文件名，否则不会复制该对象。返回一个tuple(filename,header)，其中文件名是在其中找到对象的本地文件名，而header则是urlopen()返回的对象的info()方法(对于远程对象)。异常与urlopen()相同
      print()
      statinfo = os.stat(filepath)#获取文件或文件描述符的状态。在给定的路径上执行等效的stat()系统调用。路径可以被指定为字符串或字节——直接或间接地通过PathLike接口——或者作为一个打开的文件描述符。返回一个stat_result对象
...
def ld_mnist(test_only=False):
  ...
  :param extended: include extended training data in the returned array
  :param test_only: disables loading of both train and extra -> large speed up
  :return: tuple of arrays which depend on the parameters#依赖于参数的数组元组
  """
  # Define files to be downloaded
  # WARNING: changing the order of this list will break indices (cf. below)
  ...
  # Maybe download data and retrieve local storage urls 可能下载数据并检索本地存储url
  local_urls = maybe_download(file_urls, FLAGS.data_dir)

  # Extract it into np arrays.将其提取到np数组中
...
#简单的分区算法，它返回正确的部分数据 教师需要由老师指定的老师
def partition_dataset(data, labels, nb_teachers, teacher_id):
  ...
  :param labels: output data to be partitioned#要分区的输出数据
  :param nb_teachers: number of teachers in the ensemble (affects size of each
                      partition)
  :param teacher_id: id of partition to retrieve要检索的分区id
  :return:
  """
  ...
  # This will floor the possible number of batches 这将导致可能的批次
